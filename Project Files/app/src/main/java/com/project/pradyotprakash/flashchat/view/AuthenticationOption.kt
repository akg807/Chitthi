...
fun AuthenticationView(register: () -> Unit, login: () -> Unit) {
    FlashChatTheme {
        ...
        Surface(color = Color(0xFFFFFBE9)) {
            Column(
                modifier = Modifier
                    .fillMaxWidth()
                    .fillMaxHeight(),
                ...
                Title(title = "Chitthi 🕊️")
                Buttons(title = "Login", onClick = login, backgroundColor = Color(0xFFCEAB93))
                Buttons(title = "Register", onClick = register, backgroundColor = Color(0xFFE3CAA5))
